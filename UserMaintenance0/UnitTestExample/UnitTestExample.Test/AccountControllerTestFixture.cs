...
namespace UnitTestExample.Test
{
    ...
    TestCase("irf@uni-corvinus.hu", true)
        ...
        public bool ValidatePW(string pw) {
            var lcase = new Regex(@"[a-z]+");
            var ucase = new Regex(@"[A-Z]+");
            var num = new Regex(@"[0-9]+");
            var lenght = new Regex(@".{8,}");

            return lcase.IsMatch(pw) && ucase.IsMatch(pw) && num.IsMatch(pw) && lenght.IsMatch(pw);
        }
    ...
    TestCase("irf@uni-corvinus.hu", "Ab1234"),
        ...
        public void TestRegisterValidateException(string email, string password)
        {
            ...
            try
            {
                var actuallResult = accountController.Register(email, password);
                Assert.Fail();
            }
            catch (Exception ex)
            {
